...
export class AptosNetwork extends AptosClient {
	...
	constructor(nodeUrl: string) {
		...
		this.owner = new AptosAccount(
			new HexString(
				"0x2a6f6988be264385fbfd552b8aa93451c6aac25d85786dd473fe7159f9320425", //address: 0x8ac1b8ff9583ac8e661c7f0ee462698c57bb7fc454f587e3fa25a57f9406acc0
			).toUint8Array(),
		);
		this.contractCallSequence = -1;
		this.payContractCallSequence = -1;
		this.resourceAddress =
			"0xe2a20d8c426eb04d882e20e78399b24123905d9f1adf95a292832805965e263a";
	...
	async isGatewayDeployed(): Promise<boolean> {
		try {
			...
			const resource_accounts = resources.find(
				(resource) => resource.type === "0x1::resource_account::Container",
			);
			if (!resource_accounts) return false;
			const data: any = resource_accounts.data;
			const gateway = data.store.data.find(
				(entry: any) => entry.key == this.resourceAddress,
			);
			return gateway != null;
		} catch (e) {
			return false;
	...
	async deploy(
		...
		seed: MaybeHexString | undefined = undefined,
	) {
	...
	deployAxelarFrameworkModules() {
		return this.deploy(
			path.join(
				"node_modules/@axelar-network/axelar-cgp-aptos/aptos/modules/axelar/build/AxelarFramework",
			),
	...
	queryContractCallEvents(options?: QueryOptions) {
		const _options = options || {
			start:
				this.contractCallSequence === -1 ? 0 : this.contractCallSequence + 1,
			limit: 100,
		};
