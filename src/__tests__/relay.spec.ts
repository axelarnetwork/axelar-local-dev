...
describe("relay", () => {
	...
	beforeEach(async () => {
		...
		chain1.usdc = await chain1
			.deployToken("Axelar Wrapped USDC", "aUSDC", 6, BigInt(1e18))
			.then((c: Contract) => c.connect(user1));
		chain2 = await createNetwork({ seed: "2" });
		[user2] = chain2.userWallets;
		chain2.usdc = await chain2
			.deployToken("Axelar Wrapped USDC", "aUSDC", 6, BigInt(1e18))
			.then((c: Contract) => c.connect(user2));
	});
	afterEach(async () => {
		stopAll();
	});
	describe("deposit address", () => {
		it("should generate a deposit address", async () => {
			...
			await chain1.usdc
				?.transfer(depositAddress, amount)
				.then((tx: ContractTransaction) => tx.wait());
			await relay();

			const balance = await chain2.usdc
		...
		it("should generate a deposit address to use twice", async () => {
			...
			await chain1.usdc
				?.transfer(depositAddress, amount1)
				.then((tx: ContractTransaction) => tx.wait());
			await relay();
			expect(BigInt(await chain2.usdc?.balanceOf(user2.address))).to.equal(
				amount1 - fee,
			);

			await chain1.giveToken(user1.address, "aUSDC", amount2);
			await chain1.usdc
				?.transfer(depositAddress, amount2)
				.then((tx: ContractTransaction) => tx.wait());
			await relay();
			expect(BigInt(await chain2.usdc?.balanceOf(user2.address))).to.equal(
				amount1 - fee + amount2 - fee,
			);
		});
	...
	describe("send token", () => {
		it("should send some usdc over", async () => {
			...
			await chain1.giveToken(user1.address, "aUSDC", amount);
			await chain1.usdc
				?.approve(chain1.gateway.address, amount)
				.then((tx: ContractTransaction) => tx.wait());
	...
	describe("call contract", () => {
		...
		beforeEach(async () => {
			ex1 = await deployContract(user1, Executable, [
				chain1.gateway.address,
				chain1.gasService.address,
			]);
			ex2 = await deployContract(user2, Executable, [
		...
		it("should call a contract manually and fulfill the call", async () => {
			...
			await (
				await ex2
					.connect(user2)
					.execute(args?.commandId, chain1.name, user1.address, payload)
			).wait();
		...
		it("should call a contract through the sibling and fulfill the call", async () => {
			...
			await (
				await ex2
					.connect(user2)
					.execute(args?.commandId, chain1.name, ex1.address, payload)
			).wait();
		...
		it("should have the sibling pay for gas and make the call", async () => {
			await (
				await ex1
					.connect(user1)
					.set(chain2.name, message, { value: BigInt(1e18) })
			).wait();
			await relay();
	...
	describe("call contract with token", () => {
		...
		it("should call a contract manually and fulfill the call", async () => {
			...
			expect(await ex2.sourceAddress()).to.equal(user1.address);
			expect(
				(await chain2.usdc?.balanceOf(user2.address))?.toNumber(),
			).to.equal(amount);
		});
