...
describe("aptos", () => {
	...
	it("should be able to call approve_contract_call", async () => {
		...
		const tx = await client.approveContractCall(
	...
	it("should be able to call validate_contract_call", async () => {
		...
		const packageMetadata = fs.readFileSync(
			path.join(__dirname, modulePath, "package-metadata.bcs"),
		);
		const moduleDatas = compiledModules.map((module) => {
			return fs.readFileSync(
				path.join(__dirname, modulePath, "bytecode_modules", module),
			);
		});
		...
		const approveTx = await client.approveContractCall(
			commandId,
			args[0],
			args[1],
			args[2],
		...
		const resourceType = `${client.owner
			.address()
			.hex()}::hello_world::MessageHolder`;
