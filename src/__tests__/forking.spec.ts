...
describe("forking", () => {
	...
	it.skip("should fork Avalanche mainnet", async () => {
		...
		const avalanche = chains.find(
			(chain: { name: string }) => chain.name === chainName,
		) as any;
	...
	it.skip("should fork Avalanche and Ethereum and send some USDC back and forth", async () => {
		...
		expect(
			BigInt(await avalanche.usdc.balanceOf(userAvalanche.address)),
		).to.equal(amount1);
		await (
			await avalanche.usdc
				.connect(userAvalanche)
				.approve(avalanche.gateway.address, amount1)
		).wait();
		await (
			await avalanche.gateway
				.connect(userAvalanche)
				.sendToken(
					ethereum.name,
					userEthereum.address,
					avalanche.tokens[alias],
					amount1,
				)
		).wait();
		...
		await relay();
		expect(
			BigInt(await ethereum.usdc.balanceOf(userEthereum.address)),
		).to.equal(BigInt(amount1 - fee1));
		const amount2 = amount1 - fee1;
		const fee2 = BigInt(getFee());
		await (
			await ethereum.usdc
				.connect(userEthereum)
				.approve(ethereum.gateway.address, amount2)
		).wait();
		await (
			await ethereum.gateway
				.connect(userEthereum)
				.sendToken(
					avalanche.name,
					userAvalanche.address,
					ethereum.tokens[alias],
					amount2,
				)
		).wait();
