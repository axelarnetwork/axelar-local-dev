...
describe('forking', () => {
    ...
    it('should fork Avalanche mainnet', async () => {
        ...
        const avalanche = chains.find((chain: any) => chain.name === chainName) as any;
        const chain: any = await forkNetwork(avalanche, {
            ganacheOptions: {
                fork: { deleteCache: true },
            },
        });
        chain.usdc = chain.getTokenContract(tokenAlias);
        expect(await chain.usdc.name()).to.equal('Axelar Wrapped USDC');
        const address = new Wallet(keccak256(toUtf8Bytes('random'))).address;
        await chain.giveToken(address, tokenAlias, testAmount);
    ...
    it('should fork Avalanche and Ethereum and send some USDC back and forth', async () => {
        ...
        for (const chainName of ['Avalanche', 'Ethereum']) {
            const chainInfo = chains.find((chain: any) => chain.name === chainName) as any;
