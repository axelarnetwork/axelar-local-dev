...
const {
    utils: { defaultAbiCoder }, Contract, Wallet,
} = require('ethers');
...
describe('forking', async () => {
    it('should fork Avalanche mainnet', async () => {return;
    ...
    it('should fork Avalanche and Ethereum and send some USDC back and forth', async () => {
        ...
        await (await avalanche.gateway.connect(userAvalanche).sendToken(
            ethereum.name, 
        ...
        await (await ethereum.gateway.connect(userEthereum).sendToken(
            avalanche.name, 
