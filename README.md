...
async function main() {
    ...
    const ethApproveTx = await usdcEthContract.connect(ethUserWallet).approve(eth.gateway.address, 100e6);
    await ethApproveTx.wait();

    // ask gateway on source chain to send tokens to destination chain
    const ethGatewayTx = await eth.gateway.connect(ethUserWallet).sendToken(avalanche.name, avalancheUserWallet.address, 'aUSDC', 100e6);
    await ethGatewayTx.wait();

    // relay transactions
    await relay();

    console.log((await usdcEthContract.balanceOf(ethUserWallet.address)) / 1e6, 'aUSDC');
    console.log((await usdcAvalancheContract.balanceOf(avalancheUserWallet.address)) / 1e6, 'aUSDC');
}
...
```ts
import { createNearNetwork, createNetwork, deployContract, relay, stopAll } from '@axelar-network/axelar-local-dev';

import path from 'path';

// Example of this contract can be found in axelar-local-dev/src/contracts/test/Executable.sol
import Executable from './Executable.sol/Executable.json';

async function main() {
    console.log('Starting NEAR and EVM network...');
    ...
    // Example of this contract can be found in axelar-local-dev/src/near/contracts/test/near-axelar-contract-call-example/src/lib.rs
    const nearWasmFilePath = path.join(path.resolve(__dirname), '/axelar_near_executable.wasm');
    ...
    // Relay transaction from EVM to NEAR
    await relay();

    // Get value from NEAR contract
    const nearValue = await nearContract.view('get_value', {});

    // Get source chain and source address from NEAR contract
    ...
    NEAR source chain: ${nearContractSourceChain}
    NEAR source address: ${nearContractSourceAddress}
    `);
    ...
    // Relay transaction from NEAR to EVM
    await relay();

    console.log(`
    EVM value: ${await evmContract.value()}
    EVM source chain: ${await evmContract.sourceChain()}
    EVM source address: ${await evmContract.sourceAddress()}
    `);

    // Stop all networks (NEAR and EVM)
...
Within the console logs after running the example above you would see something similar to this:
