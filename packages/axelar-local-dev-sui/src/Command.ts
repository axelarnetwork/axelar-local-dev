...
export class Command {
    ...
    static createContractCallCommand = (commandId: string, suiNetwork: SuiNetwork, relayData: RelayData, args: CallContractArgs) => {
        return new Command(
            ...
            [],
            async () => {   
                const callInfoObject = (await suiNetwork.getObject({id: args.destinationContractAddress, options: {showContent: true}}) as any);
                ...
                tx.moveCall({
                    target: infoTarget,
                    arguments: [tx.pure(String.fromCharCode(...arrayify(args.payload))), ...callObjectIds.map((id: string) => tx.object(id))],
                });
